...
def readingXMLFile(file,urlLink):
    ...
    for i in range (0,numOfINews+1):
        ...
        xmlVidID = xmlTagLink.replace('<link>https://www.youtube.com/watch?v=','').replace('&amp;feature=youtube_gdata</link>','').encode('utf-8').strip()
...
def getVidDetails(xmlVidID):
    ...
    xmlDataTitle = xmlTagTitle.replace('<title type="text">','').replace('</title>','').encode('latin-1','ignore').strip()

    
    xmlTagAbout = xmldoc.getElementsByTagName('content')[0]
    #xmlDataDescription = xmlTagAbout.firstChild.nodeValue;
    #xmlDataDescription = xmlTagAbout.firstChild.nodeValue.replace('</title>','').encode('latin-1','ignore').strip();
   
    try:
    
           xmlDataDescription = xmlTagAbout.firstChild.nodeValue.replace('','').encode('utf-8').strip()
    except AttributeError,e:
           print "--------------------------";
           xmlDataDescription ="No description available.";
    ...
    vidCategory = itemlist[1].attributes['label'].value
    print "category:", vidCategory
    ...
    xmlTagAbout = xmldoc.getElementsByTagName('content')[0]
   # xmlDataDescription = xmlTagAbout.firstChild.nodeValue;
    print "About :", xmlDataDescription
